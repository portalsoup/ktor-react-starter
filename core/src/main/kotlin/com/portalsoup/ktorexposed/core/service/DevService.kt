...
object DevService {
    fun loadData() {
        ...
        val testUser = transaction { TravelerDAO.getByEmail("test").toResource() }
        val importedRoute = GPXService.importGpx(gpx, testUser).first()

        transaction {
            BlogPostDAO.create(
                BlogPostListResource(
                    listOf(
                        BlogPostResource(
                            null,
                            testUser,
                ...
                CurrentUserResource(testUser.id ?: throw RuntimeException("Jank?"), testUser.email)
            )
        }
    }
}